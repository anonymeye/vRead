...
extension Application {
    ...
    func sendRequest<T>(to path: String, method: HTTPMethod, headers: HTTPHeaders = .init(),
        ...
        let responder = try self.make(Response.self)
    ...
    */
    ...
    func sendRequest<T>(to path: String, method: HTTPMethod, headers: HTTPHeaders = .init(),
                        body: T? = nil, loggedInRequest: Bool = false, loggedInUser: User? = nil
        ...
        if (loggedInRequest || loggedInUser != nil) {
            ...
            let tokenResponse = try self.sendRequest(to: "/users/login", method: .POST, headers: tokenHeaders)
    ...
    func sendRequest(to path: String, method: HTTPMethod, headers: HTTPHeaders = .init(),
                     loggedInRequest: Bool = false, loggedInUser: User? = nil) throws -> Response {
    ...
    return try self.getResponse(
        ...
        headers: headers,
        data: emptyContent,
        decodeTo: type, loggedInRequest: loggedInRequest, loggedInUser: loggedInUser)
    }
